...
let file = Sys.argv.(1)
let () = if ((String.length file) < 4) || not (String.equal (String.sub file ((String.length file) - 4) 4) ".exp")
...
let () = print_string ("Le fichier " ^ file_name ^ ".s peut être compilé avec la commande \"gcc -no-pie " ^ file_name ^ ".s -o " ^ file_name ^ "\".\n")
